...
class Episode(object):
    ...
    def calc_qvals(self, gamma: float) -> List[float]:
        ...
        # ====== YOUR CODE: ======
        # gamma_lst = [gamma]*len(self.experiences)
        sum_exp = 0
        for val in reversed(self.experiences):
            sum_exp *= gamma
            sum_exp += val.reward
            qvals.append(sum_exp)
...
class TrainBatch(object):
    ...
    def from_episodes(cls, episodes: Iterable[Episode], gamma=0.999):
        ...
        q_vals_tensor = torch.FloatTensor()
        total_rewards_tensor = torch.FloatTensor()

        for episode in episodes:
            curr_exp = episode.experiences
            total_rewards_tensor = torch.cat((total_rewards_tensor, torch.FloatTensor([episode.total_reward, ])), dim=0)
            q_vals_tensor = torch.cat((q_vals_tensor, torch.FloatTensor(episode.calc_qvals(gamma))), dim=0)
            for exp in curr_exp:
                row_tensor_state = torch.unsqueeze(torch.FloatTensor(exp.state), dim=0)
                states_tensor = torch.cat((states_tensor, row_tensor_state), dim=0)
                actions_tensor = torch.cat((actions_tensor, torch.LongTensor([exp.action, ])), dim=0)
        # for count, episode in enumerate(episodes):
        #     if count == 0:
        #         states_tensor = torch.stack(
        #             [epi.state for epi in episode.experiences], dim=0)
        #         actions_tensor = torch.tensor(
        #             [epi.action for epi in episode.experiences])
        #         q_vals_tensor = torch.tensor(episode.calc_qvals(gamma))
        #         total_rewards_tensor = torch.tensor(
        #             episode.total_reward).unsqueeze(dim=0)
        #     else:
        #         states_tensor = torch.cat((states_tensor, torch.stack(
        #             [epi.state for epi in episode.experiences], dim=0)), dim=0)
        #         actions_tensor = torch.cat((actions_tensor, torch.tensor(
        #             [epi.action for epi in episode.experiences])), dim=0)
        #         q_vals_tensor = torch.cat(
        #             (q_vals_tensor, torch.tensor(episode.calc_qvals(gamma))), dim=0)
        #         total_rewards_tensor = torch.cat((total_rewards_tensor, torch.tensor(
        #             episode.total_reward).unsqueeze(dim=0)), dim=0)

        train_batch = TrainBatch(
            states_tensor, actions_tensor, q_vals_tensor, total_rewards_tensor)
    ...
    def episode_batch_generator(self) -> Iterator[Tuple[Episode]]:
        ...
        while True:
            ...
            while not is_done:
                ...
                is_done = curr_exp.is_done
            curr_batch.append(Episode(episode_reward, episode_experiences))
            episode_experiences = []
            episode_reward = 0.0
            # print('finish with episode '+len(curr_batch))
            # ========================
