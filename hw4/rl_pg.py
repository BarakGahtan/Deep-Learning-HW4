...
class PolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, **kw):
        ...
        for i in kw['hidden_layers']:
            layers.append(nn.Linear(start_with, i))
            layers.append(nn.ReLU())
            start_with = i

        layers.append(nn.Linear(kw['hidden_layers'][-1], out_actions))

        # layers.append(nn.Sigmoid())
        self.extract_features = torch.nn.Sequential(*layers)
        # ========================

    def forward(self, x):
        # TODO: Implement a simple neural net to approximate the policy.
        # ====== YOUR CODE: ======
        action_scores = self.extract_features(x)
        # ========================
...
class PolicyAgent(object):
    ...
    def step(self) -> Experience:
        ...
        # ======== My Code: ==========
        actions_probability = self.current_action_distribution()
        chosen_action = torch.distributions.Categorical(actions_probability)
        index_chosen_action = int(chosen_action.sample().item())
        next_state, reward, is_done, _ = self.env.step(index_chosen_action)
        next_state = torch.tensor(next_state, device=self.device)
        self.curr_episode_reward = self.curr_episode_reward + reward
        experience = Experience(
            self.curr_state, index_chosen_action, reward, is_done)
...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def forward(self, batch: TrainBatch, action_scores: torch.Tensor, **kw):
        ...
        loss_p = self._policy_loss(
            batch, action_scores, self._policy_weight(batch))
        # ========================
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        # ====== YOUR CODE: ======
        log_probability_cal = torch.log_softmax(action_scores, dim=1)
        actions = log_probability_cal.gather(
            dim=1, index=batch.actions.view(-1, 1)).squeeze(dim=1)
        # loss_p = policy_weight.view(-1, 1) * actions
        # loss_p = -torch.mean(loss_p)
        loss_p = (-1.0/len(batch))*(actions * policy_weight).sum(dim=-1)
        # ========================
        return loss_p
...
class BaselinePolicyGradientLoss(VanillaPolicyGradientLoss):
    def forward(self, batch: TrainBatch, action_scores: torch.Tensor, **kw):
        ...
        diff_weight = weight - baseline
        loss_p = self._policy_loss(batch, action_scores, diff_weight)
        # ========================
...
class ActionEntropyLoss(nn.Module):
    ...
    def calc_max_entropy(n_actions):
        ...
        # ====== YOUR CODE: ======
        max_entropy = -1.0 * torch.log(torch.tensor(1.0/n_actions))
        # ========================
        return max_entropy
    ...
    def forward(self, batch: TrainBatch, action_scores, **kw):
        ...
        # ====== YOUR CODE: ======
        N = len(batch)
        loss_e = -1.0*(torch.log_softmax(
            action_scores, dim=1) * torch.softmax(action_scores, dim=1)).sum(dim=1)
        max_e = self.calc_max_entropy(action_scores.shape[1])
        avg_ent = (1.0/N)*loss_e.sum(dim=0)
        loss_e = avg_ent / max_e
...
class PolicyTrainer(object):
    ...
    def train_batch(self, batch: TrainBatch):
        ...
        for current_loss_function in self.loss_functions:
            current_loss, current_dictionary = current_loss_function(
                batch, action_scores)
            if total_loss is not None:
                total_loss += current_loss
            else:
                total_loss = current_loss
            losses_dict.update(current_dictionary)
        total_loss.backward()
        self.optimizer.step()
        # ========================
