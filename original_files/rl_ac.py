...
class AACPolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, **kw):
        ...
        layers2.append(nn.Linear(kw['hidden_layers'][-1], out_actions))

        # layers1.append(nn.Sigmoid())
        # layers2.append(nn.Sigmoid())
        self.extract_features1 = torch.nn.Sequential(*layers1)
        self.extract_features2 = torch.nn.Sequential(*layers2)
        # ========================
    ...
    def forward(self, x):
        ...
        #  given state.
        # ====== YOUR CODE: ======
        action_scores = self.extract_features1(x)
        state_values = self.extract_features2(x)
        # ========================
...
class AACPolicyAgent(PolicyAgent):
    ...
    def current_action_distribution(self) -> torch.Tensor:
        ...
        # ====== YOUR CODE: ======
        with torch.no_grad():
            action_scores = self.p_net(self.curr_state)[0]
...
class AACPolicyGradientLoss(VanillaPolicyGradientLoss):
    ...
    def forward(self, batch: TrainBatch, model_output, **kw):
        ...
        loss_p = self._policy_loss(
            batch, action_scores, advantage)[0]
        # print(loss_p)
        loss_v = self._value_loss(batch, state_values)[0]
        # ========================
    ...
    def _value_loss(self, batch: TrainBatch, state_values: torch.Tensor):
        ...
        N = len(batch)
        q_vals = batch.q_vals
        inner_tensor = torch.pow((state_values-q_vals), 2).sum(dim=-1)
        loss_v = (1/N)*inner_tensor
