...
class TrainBatch(object):
    ...
    def from_episodes(cls, episodes: Iterable[Episode], gamma=0.999):
        ...
        for episode in episodes:
            ...
            total_rewards_tensor = torch.cat(
                (total_rewards_tensor, torch.FloatTensor([episode.total_reward, ])), dim=0)
            q_vals_tensor = torch.cat(
                (q_vals_tensor, torch.FloatTensor(episode.calc_qvals(gamma))), dim=0)
            for exp in curr_exp:
                row_tensor_state = torch.unsqueeze(
                    torch.FloatTensor(exp.state), dim=0)
                states_tensor = torch.cat(
                    (states_tensor, row_tensor_state), dim=0)
                actions_tensor = torch.cat(
                    (actions_tensor, torch.LongTensor([exp.action, ])), dim=0)
